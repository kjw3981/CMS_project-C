...
int main(void)
{
    ...
    if (id_count == -1) {
        ...
        return 1;
            ...
            sprintf(query, "select title, upload_date, id, visit_cnt from posts where user_id = %s", session.user_id);
            // printf("select posts query<br>%s<br>", query);
            if (mysql_query(conn, query)) {
    ...
    if (id_count == 0) {
        ...
        for (int i = 0; i < id_count; i++) {
            ...
            if (check_post_id(conn, &result, &row, char_post_id) == 0){
              ...
              printf("<h3>%s　%s</h3>", articleNew[i].upload_date, articleNew[i].visit_cnt);
          }
