...
int main(void)
{
    ...
    printf("</div><br><h3>お問い合わせ情報</h3><div class=\"sortable-table\"><table id=\"order_aiueo\">");
    ...
    sprintf(query, "SELECT COUNT(*) from inquiry;"); // countもってくる
    ...
    sprintf(query, "SELECT upload_date, user_id, contents, title FROM inquiry;"); // upload_date, user_id, contents, title持ってくる
    if (mysql_query(conn, query)) {
        fprintf(stderr, "Query execution failed: %s\n", mysql_error(conn));
        mysql_close(conn);
        return 1;
...
// 管理者へのお問い合わせ確認する際、タイトルクリックするとページ偏移でなく画面上部にポップアップされる。
    printf("<script>");
    for (int i = 0; i < id_count; i++) {
